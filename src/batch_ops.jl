...
@generated function _get_archetypes(world::World, filter::F) where {F<:Filter}
        ...
        :(
            new_table_tuple =
                _find_or_create_table!(
                    world, batch.table, $add_ids, $rem_ids, $rel_ids, targets, $add_mask, $rem_mask,
                    $use_map,
                    $world_has_rel,
                )
        ),
...
@generated function _remove_entities!(
    ...
    filter::F,
    ::HFN,
) where {Fn,W<:World,F<:Filter,HFN<:Val}
...
@generated function _new_entities!(
    ...
    if length(types) > 0 && DEF === Val{true}
        ...
        for i in 1:length(types)
            ...
            push!(body_exprs.args, :($cols_sym = _get_component_columns(world, $T)))
...
@generated function _get_columns(
    ...
    for i in 1:length(comp_types)
        ...
        push!(exprs, :(@inbounds $cols_sym = _get_component_columns(world, $(comp_types[i]))))
