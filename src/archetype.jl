...
mutable struct _Archetype{M}
    ...
    const index::Vector{_Linear_Map{UInt32,_IdCollection}}
    const target_tables::_Linear_Map{UInt32,_IdCollection}
...
function _Archetype(id::UInt32, node::_GraphNode, table::UInt32)
    _Archetype(
        Memory{Int}(),
        _IdCollection(table),
        Vector{_Linear_Map{UInt32,_IdCollection}}(),
        _Linear_Map{UInt32,_IdCollection}(),
        Vector{UInt32}(),
        node,
        UInt32(0),
        table,
...
function _Archetype(
    ...
    _Archetype(
        ...
        _IdCollection(),
        [_Linear_Map{UInt32,_IdCollection}() for _ in eachindex(relations)],
        _Linear_Map{UInt32,_IdCollection}(),
        Vector{UInt32}(),
        node,
