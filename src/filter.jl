...
@generated function _Filter_from_types(
    ...
    required_ids = Int[_component_id(CS, C) for C in required_types]
    with_ids = Int[_component_id(CS, C) for C in with_types]
    without_ids = Int[_component_id(CS, C) for C in without_types]
    non_exclude_ids = Int[_component_id(CS, C) for C in non_exclude_types]
    rel_ids = Int[_component_id(CS, C) for C in rel_types]
