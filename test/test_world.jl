...
@static if RUN_JET
    @testset "World new_entity! JET" begin
        ...
        rep = JET.@report_opt function_filter = function_filter ignored_modules = (AnyFrameModule(Mmap), AnyFrameModule(Base.Filesystem)) new_entity!(world, (Position(1, 2), Velocity(3, 4)))
...
@static if RUN_JET
    @testset "World new_entities! JET" begin
        ...
        rep = JET.@report_opt function_filter = function_filter ignored_modules = (AnyFrameModule(Mmap), AnyFrameModule(Base.Filesystem)) new_entities!(
            world,
            100,
...
@static if RUN_JET
    @testset "World add/remove component JET" begin
        ...
        e1 = new_entity!(world, ())
        rep = JET.@report_opt function_filter = function_filter ignored_modules = (AnyFrameModule(Mmap), AnyFrameModule(Base.Filesystem)) add_components!(
            world,
            e1,
